	...
	return (
		<nav className=" flex w-full justify-between items-center h-20 px-4 absolute z-10">
			<div className=" ">
				<h1
					onClick={handelNav}
					className={`${logo ? "hidden" : "block"} uppercase text-white`}>
					Beaches.
				</h1>
			</div>
			<div>
			...
			<div onClick={handelNav} className=" md:hidden z-10">
				{nav ? (
					...
					<HiOutlineMenu className=" text-white" size={20} />
				)}
			</div>
			...
			<nav
				...
				className={
					nav
						? "absolute left-[0]  transition-all duration-[0.5s]  ease-in-out top-0 w-full bg-gray-100/90 px-4 py-7 flex flex-col"
						: "absolute left-[0] top-[-650%]  transition-all duration-[0.5s]  ease-in-out w-full bg-gray-100/90 px-4 py-7 flex flex-col"
				}>
				<ul className="">
